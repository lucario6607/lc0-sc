...
class BackendHandler {
 public:
  ...
  BackendAttributes GetAttributes() {
    assert(backend_);
    auto capabilities = backend_->GetCapabilities();
    BackendAttributes attrs{
        .has_mlh = capabilities.has_mlh(),
        .has_wdl = capabilities.has_wdl(),
 ...
 private:
  ...
  // Backend instance.
  std::unique_ptr<Network> backend_;
  // Worker threads.
...
class ServerConnection
 ...
 private:
  ...
  typename Archive::ResultType HandleMessage(const client::Handshake& message,
    ...
    auto iter = message.network_name_ == kAutoDiscover
...
void BackendHandler::Worker() {
  ...
  try {
    while (true) {
      ...
      auto computation = backend_->NewComputation(0);
      // Add inputs to the backend computation.
      {
        ...
        for (const auto& item : batch) {
          for (size_t i = item.first_; i < item.last_; ++i) {
            ...
            position.base_ = history.Last();
            int& transform = position.transform_;
            FillEmptyHistory fill =
                static_cast<FillEmptyHistory>(position.history_fill_);
            bool swap_colors = position.swap_colors_;
            auto planes =
                EncodePositionForNN(backend_->GetCapabilities().input_format,
                                    history, 8, fill, &transform, swap_colors);
      ...
      for (const auto& item : batch) {
        for (size_t i = item.first_; i < item.last_; ++i) {
          auto legal_moves = item.computation_->GetInput(i)
                                 .base_.GetBoard()
                                 .GenerateLegalMoves();
          int transform = item.computation_->GetInput(i).transform_;
          auto eval_result =
              item.computation_->GetEvalResult(i, legal_moves.size());
          *eval_result.q = computation->GetQVal(i - item.first_);
          *eval_result.d = computation->GetDVal(i - item.first_);
          *eval_result.m = computation->GetMVal(i - item.first_);
          size_t j = 0;
          for (Move move : legal_moves) {
            eval_result.p[j++] = computation->GetPVal(
                i - item.first_, move.as_nn_index(transform));
          }
        }
      }
...
void SharedQueue::StartServer() {
  if (options_->Get<std::string>(kProtocolOptionId) == "unix") {
    ...
    BackendServer<asio::ip::tcp> server(io_context_, *options_);
    io_context_.run();
  } else {
...
class BackendserverEngine final : public UciLoop {
 public:
...
void RunBackendServer() {
  ...
  NetworkFactory::PopulateOptions(&options_parser);
  ...
  try {
    ...
    ConsoleThread console([&] {
      ...
      std::string line;
      BackendserverEngine engine{options_parser};
