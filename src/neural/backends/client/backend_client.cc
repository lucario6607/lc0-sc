...
template <typename Proto>
class BackendClient final : public Network {
 public:
  BackendClient(const std::string& network, const OptionsDict& options)
    ...
    auto attrs = connections_.front().GetAttributes();
    using NF = pblczero::NetworkFormat;
    caps_.input_format = static_cast<NF::InputFormat>(attrs.input_format);
    caps_.use_history_input = true;
    caps_.moves_left = attrs.has_mlh ? NF::MOVES_LEFT_V1 : NF::MOVES_LEFT_NONE;
    caps_.output_format = attrs.has_wdl ? NF::OUTPUT_WDL : NF::OUTPUT_CLASSICAL;
  }
  ~BackendClient() override {}

  const NetworkCapabilities& GetCapabilities() const override { return caps_; }

  BackendAttributes GetAttributes() const {
    return connections_.front().GetAttributes();
  }
  std::unique_ptr<NetworkComputation> NewComputation(
  ...
  int GetThreads() const override {
    return GetAttributes().suggested_num_search_threads;
  }

  bool IsCpu() const override { return GetAttributes().runs_on_cpu; }
 ...
 private:
  ...
  NetworkCapabilities caps_{};
};

template <typename Proto>
class BackendClientComputation final : public NetworkComputation {
 public:
  struct Result {
    Result() = default;
    Result(unsigned moves) : p_(moves, 0.0f) {}
    float q_ = 0.0f;
    float d_ = 0.0f;
    float m_ = 0.0f;
    std::vector<float> p_;
  ...
  int GetBatchSize() const override { return entries_.size(); }

  void AddInput(InputPlanes&&) override {
    throw Exception(
        "AddInput with InputPlanes not supported in client backend");
  }

  void AddInput(const PositionHistory& pos, unsigned num_legal_moves,
                unsigned fill, bool swap) override {
    InputPosition input_pos;
    input_pos.history_fill_ = fill;
    input_pos.swap_colors_ = swap;
    size_t len = std::min(pos.GetLength(), kMoveHistory);
    input_pos.history_length_ = len - 1;
    input_pos.base_ = pos.GetPositionAt(pos.GetLength() - len);
    for (size_t i = 1; i < len; ++i) {
      // Convert history planes to moves for smaller message size.
      Move move =
          pos.GetPositionAt(pos.GetLength() - len + i - 1)
              .GetNextMove(pos.GetPositionAt(pos.GetLength() - len + i));
      input_pos.history_[i - 1] = move;
    }
    entries_.emplace_back(input_pos, num_legal_moves);
  }
  ...
  Result& GetResult(size_t index) { return entries_[index].values_; }

  float GetQVal(int idx) const override { return entries_[idx].values_.q_; }

  float GetDVal(int idx) const override { return entries_[idx].values_.d_; }

  float GetMVal(int idx) const override { return entries_[idx].values_.m_; }

  float GetPVal(int idx, int move) const override {
    return entries_[idx].values_.p_[move];
  }
 ...
 private:
  ...
  struct Entry {
    Entry() = default;
    Entry(const InputPosition& pos, unsigned moves)
        : pos_(pos), values_(moves) {}
...
typename Archive::ResultType ClientConnection<Proto>::HandleMessage(
  ...
  if (message.results_.size() !=
      static_cast<unsigned>(computation->GetBatchSize())) {
    CERR << "Received ComputeBlockingReply with unexpected number of "
            "results: "
         << message.results_.size() << " expected "
         << computation->GetBatchSize();
    return Unexpected(ArchiveError::InvalidData);
  }

  for (size_t i = 0; i < message.results_.size(); ++i) {
    const auto& net_result = message.results_[i];
    auto& result_ptr = computation->GetResult(i);
    result_ptr.q_ = net_result.value_;
    result_ptr.d_ = net_result.draw_;
    result_ptr.m_ = net_result.moves_left_;
    if (!result_ptr.p_.empty()) {
      if (net_result.policy_.size() != result_ptr.p_.size()) {
...
template <typename Proto>
std::unique_ptr<NetworkComputation> BackendClient<Proto>::NewComputation(
    size_t time_remaining) {
...
std::unique_ptr<Network> MakeClientNetwork(const std::optional<WeightsFile>&,
                                           const OptionsDict& client_options) {
  auto network =
      client_options.GetOrDefault("network", std::string(kAutoDiscover));
...
REGISTER_NETWORK("client", MakeClientNetwork, -1);
