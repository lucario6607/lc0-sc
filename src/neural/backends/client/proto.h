...
struct BackendAttributes {
  ...
  typename Archive::ResultType Serialize(Archive& ar, const unsigned) {
    ...
    r = r.and_then([this](Archive& ar) { return ar & input_format; });
    return r;
  }
};
...
struct InputPosition {
  Position base_{};
  int transform_;
  bool history_fill_{false};
  bool swap_colors_{false};
  ...
  typename Archive::ResultType Serialize(
      Archive& ar, [[maybe_unused]] const unsigned version) {
    auto r = ar & base_;
    r = r.and_then([this](Archive& ar) { return ar & history_fill_; });
    r = r.and_then([this](Archive& ar) { return ar & swap_colors_; });
 ...
 private:
  ...
  void ParseInput(MessageCallback&& callback) {
    ...
    BinaryIArchive ia(std::span<const char>(first, InputEnd()), input_.data(),
                      0, kBackendApiVersion);
    size_t size = ia.Size();
    auto rv = ParseMessage(ia, pending_header_,
                           std::forward<MessageCallback>(callback));
    if (!rv) {
      CERR << "Error parsing message "
           << static_cast<uint32_t>(pending_header_.type_)
           << " body: " << rv.error();
      Close();
      return;
    }
